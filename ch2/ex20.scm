...
(define (same-parity first-arg . num-list)
  (define (same-parity-iter first-arg num-list parity-list)
    (if (null? num-list)
        (cons first-arg parity-list)
        (if (or (and (even? first-arg)
                     (even? (car num-list)))
                (and (odd? first-arg)
                     (odd? (car num-list))))
            (same-parity-iter first-arg
                              (cdr num-list)
                              (append parity-list (list (car num-list))))
            (same-parity-iter first-arg
                              (cdr num-list)
                              parity-list))))
  (same-parity-iter first-arg num-list '()))

(same-parity 4 9 11 12 18 20) ; => (4 12 18 20)
