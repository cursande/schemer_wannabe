...
;; administrative information from division files, is dismayed to discover that, although all the division
;; files have been implemented as data structures in Scheme, the particular data structure used varies
;; from division to division. A meeting of division managers is hastily called to search for a strategy
...
(define (get-record employee personnel-file)
  (let ((division (division personnel-file))
        (personnel-contents (contents personnel-file)))
    ((get 'get-record division) employee personnel-contents)))
